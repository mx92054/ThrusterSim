...
static u8*	isr_buf ;							//中断程序缓冲

static u8		pos_msg = 0 ;					//接收指针
static u8		frame_len ;						//命令帧长度
static u8		trans_len ;						//响应帧长度
static u8		bFrameStart = 0 ;		 	//开始帧响应	
static u8		uFrameInterval = 5 ;	//帧间隙	

static __IO u16	nMBInterval = 0 ;		// interval timer 
//-------------------------------------------------------------------------------
//	@brief	中断初始化
//	@param	None
//	@retval	None
...
void		Modbus_init(void)
{
	...
	{
		case 96:
			uFrameInterval = 15 ;		break ;
		case 192:
			uFrameInterval = 10 ; 	break ;
		case 384:
			uFrameInterval = 5 ; 		break ;
		case 1152:
			uFrameInterval = 5 ; 		break ;
		default:
			uFrameInterval = 10 ; 	break ;
	...
	sprintf(msg,"\r\nStation No: %d, Baudrate: %d",ADDRESS,BAUDRATE*100) ;
...
void  Modbus_task(void)
{
	...
	if ( nMBInterval > uFrameInterval )				//通信帧间隔时间已到，缓冲区域切换
		{			
			ptr = tsk_buf ;
			tsk_buf = isr_buf ;
			isr_buf = ptr ;
...
static void  Normal_response(void)
{
	...
	{
		USART_SendData(MODBUS_USARTx,tsk_buf[i]) ;
